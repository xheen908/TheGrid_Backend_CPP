...
bool GMCommands::handleCommand(uWS::WebSocket<false, true, PerSocketData>* ws, const std::string& cmd, const std::vector<std::string>& args, const std::string& fullMsg) {
    ...
    if (cmd == "level") {
        ...
        if (args.empty()) {
            ws->send(json{{"type", "chat_receive"}, {"mode", "system"}, {"message", "Benutzung: /info [Spielername]"}}.dump(), uWS::OpCode::TEXT);
            return true;
        }
        ...
        if (targetP) {
            ...
            ws->send(json{{"type", "chat_receive"}, {"mode", "system"}, {"message", "Spieler \"" + targetName + "\" wurde nicht gefunden."}}.dump(), uWS::OpCode::TEXT);
        }
        return true;
    }
    else if (cmd == "target") {
        ...
        ws->send(json{{"type", "chat_receive"}, {"mode", "system"}, {"message", "/w, /level, /tele, /invisible, /gm, /tp, /kick, /move, /info, /target"}}.dump(), uWS::OpCode::TEXT);
        return true;
    }
